...
void printLattice(const int* const* const* const* lattice, int lattice_size) {
	std::cout << "lattice:";
	for (int i = 1; i < lattice_size + 1; i++) {
		std::cout << std::endl;
		for (int j = 1; j < lattice_size + 1; j++) {
			for (int k = 1; k < lattice_size + 1; k++) {
				std::cout << (*lattice[i][j][k] == -1 ? " -1 " : std::string("  ") + std::to_string(*lattice[i][j][k]) + " ");
			}
			std::cout << std::endl;
		}
	}
	std::cout << "end lattice" << std::endl;
}
...
void walk(int steps, int steps_between_swaps, int flips_per_iteration, double k_times_T, int lattice_size, std::promise<double*>* result_promise, Simulator* sim) {
	...
	// 1 is up, -1 is down
	// note: all faces are pointers to the same ints represented by the opposite face
	// edges are NOT initialized
	int**** lattice = new int***[lattice_size + 2];
	for (int i = 1; i < lattice_size + 1; i++) {
		lattice[i] = new int**[lattice_size + 2];
		for (int j = 1; j < lattice_size + 1; j++) {
			lattice[i][j] = new int*[lattice_size + 2];
			for (int k = 1; k < lattice_size + 1; k++) {
				lattice[i][j][k] = new int(1);
			}

			// initialize edge cases to finish row ij
			lattice[i][j][0] = lattice[i][j][lattice_size];
			lattice[i][j][lattice_size + 1] = lattice[i][j][1];
		}

		// initialize edge cases to finish layer i
		lattice[i][0] = new int*[lattice_size + 2];
		lattice[i][lattice_size + 1] = new int*[lattice_size + 2];
		for (int k = 1; k < lattice_size + 1; k++) {
			lattice[i][0][k] = lattice[i][lattice_size][k];
			lattice[i][lattice_size + 1][k] = lattice[i][1][k];
		}
	}
	...
	for (int j = 1; j < lattice_size + 1; j++) {
		lattice[0][j] = new int*[lattice_size + 2];
		lattice[lattice_size + 1][j] = new int*[lattice_size + 2];

		for (int k = 1; k < lattice_size + 1; k++) {
			lattice[0][j][k] = lattice[lattice_size][j][k];
			lattice[lattice_size + 1][j][k] = lattice[1][j][k];
		}
	}
	...
	for (int step = 0; step < steps; step++, current_run_counter++) {
		...
		int delta_U = 0;
		int delta_num_up = 0;
		for (int flip = 0; flip < flips_per_iteration; flip++, current_point = (current_point + 1)%(lattice_size * lattice_size * lattice_size)) {
			int current_i = current_point / (lattice_size * lattice_size);
			int current_j = (current_point % (lattice_size * lattice_size)) / lattice_size;
			int current_k = (current_point % (lattice_size * lattice_size)) % lattice_size;
			// if they're in the same state initially, the energy goes up
			// otherwise, it goes down
			delta_U += (*lattice[current_i + 1][current_j + 1][current_k + 1] == *lattice[current_i + 1 + 1][current_j + 1][current_k + 1] ? 2 : -2);
			delta_U += (*lattice[current_i + 1][current_j + 1][current_k + 1] == *lattice[current_i - 1 + 1][current_j + 1][current_k + 1] ? 2 : -2);
			delta_U += (*lattice[current_i + 1][current_j + 1][current_k + 1] == *lattice[current_i + 1][current_j + 1 + 1][current_k + 1] ? 2 : -2);
			delta_U += (*lattice[current_i + 1][current_j + 1][current_k + 1] == *lattice[current_i + 1][current_j - 1 + 1][current_k + 1] ? 2 : -2);
			delta_U += (*lattice[current_i + 1][current_j + 1][current_k + 1] == *lattice[current_i + 1][current_j + 1][current_k + 1 + 1] ? 2 : -2);
			delta_U += (*lattice[current_i + 1][current_j + 1][current_k + 1] == *lattice[current_i + 1][current_j + 1][current_k + 1 - 1] ? 2 : -2);

			// debugging
			// std::cout << current_point << std::endl;
			// std::cout << current_i << std::endl;
			// std::cout << current_j << std::endl;
			// std::cout << current_k << std::endl;
			// flip the particle
			*lattice[current_i + 1][current_j + 1][current_k + 1] = -1 * (*lattice[current_i + 1][current_j + 1][current_k + 1]);
			delta_num_up += *lattice[current_i + 1][current_j + 1][current_k + 1];
		}
		...
		if (delta_U <= 0 || nextrand < exp(-delta_U / k_times_T)) {
			...
			U += delta_U;
			num_up += delta_num_up;
			magnetization = num_up - (lattice_size * lattice_size * lattice_size - num_up);
		} else {
			// reject
			// std::cout << "rejected" << std::endl;
			for (int flip = (lattice_size * lattice_size * lattice_size + current_point - flips_per_iteration)%(lattice_size * lattice_size * lattice_size);
						flip != current_point;
						flip = (flip + 1)%(lattice_size * lattice_size * lattice_size))
			{
				int current_i = flip / (lattice_size * lattice_size);
				int current_j = (flip % (lattice_size * lattice_size)) / lattice_size;
				int current_k = (flip % (lattice_size * lattice_size)) % lattice_size;
				// debugging
	...
	for (int i = 1; i < lattice_size + 1; i++) {
		...
		for (int j = 1; j < lattice_size + 1; j++) {
			for (int k = 1; k < lattice_size + 1; k++) {
				delete lattice[i][j][k];
			}
			delete[] lattice[i][j];
		}
		delete[] lattice[i][lattice_size + 1];
	}
...
int main(int argc, char const *argv[]) {
	...
	// write data to file
	std::ofstream data_file ("ising_3d.dat");
	for (int i = 0; i < num_of_temps; i++) {
